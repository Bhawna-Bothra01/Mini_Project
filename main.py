...
async def predict_voice_language(file: UploadFile = File(...)):
    try:
        if file.content_type not in ["audio/mpeg", "audio/wav"]:
            raise HTTPException(status_code=400, detail="Invalid file type. Only MP3 and WAV are supported.")

        audio_data, sample_rate = librosa.load(io.BytesIO(await file.read()), sr=None)
        features = extract_features(audio_data, sample_rate)
        prediction = voice_model.predict(features)
        predicted_language = lang_label_dict.get(prediction[0], "Unknown Language")
        return JSONResponse(content={"predicted_language": predicted_language})
...
@app.post("/predict_code_switch/")
async def get_code_switch_language_predictions(request: SentenceRequest):
    ...
    return {"languages": language_names_list}

@app.post("/predict_text_language/")
async def predict_text_language(input: TextInput):
    try:
        preprocessed_text = preprocess(input.text)
        text_tfidf = tfidf_vectorizer.transform([preprocessed_text])
        prediction = svm_classifier.predict(text_tfidf)
        return {"predicted_language": prediction[0]}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
